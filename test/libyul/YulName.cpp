...
BOOST_AUTO_TEST_CASE(repository_with_blank_dialect)
{
	...
	BOOST_CHECK(!nameRepository.evmDialect());
	...
	BOOST_CHECK(nameRepository.predefined().datasize == std::nullopt);
	BOOST_CHECK(nameRepository.predefined().dataoffset == std::nullopt);
	BOOST_CHECK(nameRepository.predefined().selfdestruct == std::nullopt);
	BOOST_CHECK(nameRepository.predefined().memoryguard == std::nullopt);
	BOOST_CHECK(nameRepository.predefined().eq == std::nullopt);
	BOOST_CHECK(nameRepository.predefined().add == std::nullopt);
	BOOST_CHECK(nameRepository.predefined().sub == std::nullopt);
	BOOST_CHECK(nameRepository.predefined().tstore == std::nullopt);
...
BOOST_AUTO_TEST_CASE(repository_with_evm_dialect)
{
	...
	BOOST_REQUIRE(nameRepository.hashFunction(type).has_value());
...
BOOST_AUTO_TEST_CASE(repository_with_typed_evm_dialect)
{
	...
	YulNameRepository nameRepository(dialect);
	BOOST_CHECK(nameRepository.evmDialect());
	BOOST_REQUIRE(nameRepository.predefined().memoryguard != std::nullopt);
	...
	BOOST_REQUIRE(wordType.has_value());
	BOOST_REQUIRE(boolType.has_value());
	...
	for (auto const type : {wordType.value(), boolType.value()})
	{
		YulString typeLabel(std::string(nameRepository.labelOf(type).value()));
		BOOST_CHECK(extractEVMFunction(nameRepository.discardFunction(type)) == dialect.discardFunction(typeLabel));
		BOOST_CHECK(extractEVMFunction(nameRepository.equalityFunction(type)) == dialect.equalityFunction(typeLabel));
