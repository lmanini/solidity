...
public:
	...
	struct PredefinedHandles
	{
		...
		std::optional<YulName> datasize{}; ///< Datasize builtin.
		std::optional<YulName> dataoffset{}; ///< Dataoffset builtin.
		std::optional<YulName> selfdestruct{}; ///< Selfdestruct builtin.
		std::optional<YulName> memoryguard{}; ///< Memoryguard builtin.
		std::optional<YulName> eq{}; ///< Equality function.
		std::optional<YulName> add{}; ///< Addition function.
		std::optional<YulName> sub{}; ///< Subtraction function.
		std::optional<YulName> tstore{}; ///< Tstore builtin.
	...
	bool isVerbatimFunction(YulName const& _name) const;

	/// A couple of predefined names.
	PredefinedHandles const& predefined() const { return m_predefined; }

	/// Functionality that decorates a yul dialect builtin based on YulNames (ids).
	BuiltinFunctionWrapper const* builtin(YulName const& _name) const;
	...
	std::optional<YulName> hashFunction(YulName const& _name) const;
	...
	/// Same as nameOfLabel, but restricted to builtins and therefore computationally more efficient. Does not
	/// search for instances of verbatim.
	std::optional<YulName> nameOfBuiltin(std::string_view builtin) const;

	/// Same as nameOfLabel, but restricted to types and therefore computationally more efficient.
	std::optional<YulName> nameOfType(std::string_view type) const;

	/// Whether the name is corresponding to a type.
	bool isType(YulName const& _name) const;
...
private:
	struct PredefinedBuiltinFunctions
	{
		...
		std::vector<std::optional<YulName>> hashFunctions;
	};
